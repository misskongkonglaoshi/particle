classdef HeatingStage < handle
    ...
    methods
        ...
        function [t_history, T_history, t_end] = solve(obj, particleState, t_start)
            ...
            % 设置事件函数，当温度达到点火温度时停止
            options = odeset('Events', @(t, y) obj.reach_ignition_point_event(t, y));
        ...
        function [value, isterminal, direction] = reach_ignition_point_event(obj, ~, y)
            % 事件函数: 当温度达到点火温度时触发
            value = y(1) - obj.params.materials.Mg.ignition_temp;
